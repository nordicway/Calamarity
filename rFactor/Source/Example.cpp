...
void ExampleInternalsPlugin::openSocket()
{
	//Initialise winsock
...
float ExampleInternalsPlugin::kelvinToCelsius(float kelvin)
{
...
void ExampleInternalsPlugin::UpdateTelemetry( const TelemInfoV2 &info )
{
  ...
  // is working, and b) explain the coordinate system a little bit (see header for more info)
	...
	for(int i=0;i < 4; i++) {
		...
		//if( fo != NULL )
		//{
		//	fprintf(fo, "Wheel %d L/C/R: %f / %f / %f", i, info.mWheel[i].mTemperature[0], info.mWheel[i].mTemperature[1], info.mWheel[i].mTemperature[2]);
		//}
		//fclose(fo);



		maxTemps[i] = ( kelvinToCelsius(info.mWheel[i].mTemperature[0]) + kelvinToCelsius(info.mWheel[i].mTemperature[1]) + kelvinToCelsius(info.mWheel[i].mTemperature[2]) ) / 3;
	}
	...
	ss << "{";
	ss << "\"lap\": " << info.mLapNumber << ",";
	ss << "\"track\": " << "\"" << info.mTrackName << "\"" << ",";
	ss << "\"gear\": " << info.mGear << ",";
	ss << "\"speed\": " << metersPerSec * 3.6f << ",";
	...
	ss << "\"damage\": [";
		ss << info.mDentSeverity[0]*50 << ",";
		ss << info.mDentSeverity[1]*50 << ",";
		ss << info.mDentSeverity[2]*50 << ",";
		ss << info.mDentSeverity[3]*50 << ",";
		ss << info.mDentSeverity[4]*50 << ",";
		ss << info.mDentSeverity[5]*50 << ",";
		ss << info.mDentSeverity[6]*50 << ",";
		ss << info.mDentSeverity[7]*50;
	ss << "]";
	...
	if (sendto(s, ss.str().c_str(), strlen(ss.str().c_str()) , 0 , (struct sockaddr *) &si_other, slen) == SOCKET_ERROR)
    {
		FILE *fo = fopen( "ExampleInternalsTelemetryOutput.txt", "a" );
		if( fo != NULL )
		{
			fprintf(fo, "sendto() failed with error code : %d" , WSAGetLastError());	
		}
        //exit(EXIT_FAILURE);
